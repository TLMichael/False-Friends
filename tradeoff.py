...
def train(args, model, optimizer, loader, writer, epoch):
    ...
    for i, (inp, target) in iterator:
        ...
        desc = 'Train Epoch: {} | Beta {:.0f} | Loss {:.4f} | Accuracy {:.4f} ||'.format(epoch, args.beta, loss_logger.avg, acc_logger.avg)
...
def train_model(args, model, optimizer, schedule, train_loader, test_loader, writer):
    ...
    for epoch in range(args.epochs):
        ...
        if should_log or last_epoch:
            cln_test_loss, cln_test_acc, _ = natural_attack(args, model, test_loader, writer, epoch)
            adv_test_loss, adv_test_acc, _ = adv_attack(args, model, test_loader, writer, epoch, 'test')
            hyp_test_loss, hyp_test_acc, _ = hyp_attack(args, model, test_loader, writer, epoch, 'test')

            hyp_risk = (hyp_test_acc - cln_test_acc) / (100 - cln_test_acc)
            if writer is not None:
                writer.add_scalar('hyp_risk', hyp_risk, epoch)
            ...
            checkpoint = {
                ...
                'hyp_test_acc': hyp_test_acc,
                'hyp_test_loss': hyp_test_loss,
            }
            torch.save(checkpoint, args.model_path)
        schedule.step()
    return model

def eval_model(args, model, test_loader, flag=None):
    model.eval()
    args.num_steps = 20
    ...
    keys = ['model', 'beta', nat_name, adv_name, hyp_name, 'R_hat_adv', 'R_hat_hyp']
    values = [args.model_path, args.beta, nat_test_acc, adv_test_acc, hyp_test_acc, R_hat_adv, R_hat_hyp]
    
    import csv
    csv_fn = '{}.csv'.format(args.model_path)
    if flag is not None:
        csv_fn = '{}.{}.csv'.format(args.model_path, flag)
